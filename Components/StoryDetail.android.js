...
import XMLToReactMap from '../XMLToReactMap';
...
export default class StoryDetail extends React.Component {
    ...
    parseXMLBody(body, cb) {
        const handler = new Tautologistics.NodeHtmlParser.DefaultHandler(function (error, dom) {
            cb(dom)
    ...
    componentDidMount() {
        this.fetchStoryData((result, media) => {
            ...
            XMLToReactMap.createReactElementsWithXMLRoot(rootElement, media)
                .then(array => {
                    const scroll = React.createElement(ScrollView, {
